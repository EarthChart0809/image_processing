...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        printf("Usage: %s input.bmp output.bmp mode [param]\n", argv[0]);
        return 1;
    }
    ...
    switch (mode) {
        case 1: // 二値化
            if (argc < 5) { printf("二値化には 閾値 が必要です\n"); return 1; }
            binarize(image, width, height, (int)param);
            break;

        case 2: // 平滑化
            if (argc < 5) { printf("平滑化には 閾値 が必要です\n"); return 1; }
            averaging(image, width, height, (int)param);
            break;

        case 3: // エッジ検出
            if (argc < 5) { printf("エッジ検出には 閾値 が必要です\n"); return 1; }
            edgeDetectionFilter(image, width, height, (int)param);
            break;

        case 4: // ラプラシアン
            if (argc < 5) { printf("ラプラシアンには 閾値 が必要です\n"); return 1; }
            laplacianFilter(image, width, height, (int)param);
            break;

        case 5: // ガウシアン
            if (argc < 5) { printf("ガウシアンには sigma が必要です\n"); return 1; }
            GaussianFilter(image, width, height, param);
            break;

        case 6: // LoG
            if (argc < 5) { printf("LoG には sigma が必要です\n"); return 1; }
            LoGFilter(image, width, height, param);
            break;
